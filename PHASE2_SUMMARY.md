# GigShield AI - Phase 2 Implementation Summary
...
**Key Innovation:** Users see their exact premium before registration, calculated using ML based on their city, vehicle type, and current season.
...
**Key Innovation:** Plans automatically adjust coverage based on user's risk profile and city.
...
- [`backend/server.js`](backend/server.js) - Premium calculation logic
- [`backend/ai/risks_model.py`](backend/ai/risks_model.py) - ML models

**Features Implemented:**

- **ML-Powered Pricing:** Gradient Boosting Regressor model
- **Hyper-local Risk Factors:**
  - City risk profiles (Mumbai: 1.4x, Delhi: 1.5x, Pune: 0.8x)
  - Seasonal adjustments (Monsoon: 1.3x, Winter: 0.9x, Summer: 1.1x)
  - Vehicle discounts (EV: 15% off, Bicycle: 30% off, Scooter: 5% off)
...
**Key Innovation:** The system charges ₹2 less per week if the worker operates in a zone historically safe from water logging, or dynamically offers increased coverage hours based on predictive weather modelling.
...
**Key Innovation:** Claims are automatically created, verified, and paid without any user interaction. Average processing time: < 3 minutes.
...
- **7-Day Forecast:** Predicts disruption probability for next 7 days
...
- [`backend/server.js`](backend/server.js) - Enhanced with automated triggers, dynamic pricing, zero-touch claims
- [`backend/ai/risks_model.py`](backend/ai/risks_model.py) - Complete ML implementation with Flask API

### Frontend

- [`frontend/src/components/Dashboard.jsx`](frontend/src/components/Dashboard.jsx) - Real-time dashboard with alerts
- [`frontend/src/components/Register.jsx`](frontend/src/components/Register.jsx) - Dynamic premium calculator

### Documentation

- [`PHASE2_README.md`](PHASE2_README.md) - Comprehensive Phase 2 documentation
- [`PHASE2_SUMMARY.md`](PHASE2_SUMMARY.md) - This summary document
